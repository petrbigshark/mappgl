...
```mermaid
flowchart TD
    A["POST /run (request.json)"] --> B["Проверка входных параметров"]
    B --> C["Prefilter workbook"]
    C --> C1["prefiltered_input.xlsx"]
    C --> C2["deleted_rows.xlsx"]
    C1 --> D["Матчинг строк по модулям (brand/color/material/category/season/size)"]
    D --> E["Запуск модулей на prefiltered_input.xlsx"]
    E --> F["Сбор output-файлов модулей в run_dir"]
    F --> G["Подсчёт покрытия и rows_without_any_mapping.xlsx"]
    G --> H["run_report.json (summary + timings + module reports)"]
```
